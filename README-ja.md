...
[Releases](https://github.com/yusei531642/vibe-editor2/releases/latest) ページから最新の Windows インストーラを落とすのが最速です。
...
- チーム状態の永続化: 作成したチームは `~/.vibe-editor2/team-history.json` に保存。**履歴 → Teams** からワンクリックで復元、各メンバーは `claude --resume <session>` で過去の会話をそのまま継続
